...
int runTests (void)
{
    ...
    for (i = 0, failCount = 0; i < LENGTH_OF_ARRAY(tests); i++)
    {
        ...
        printf("SHA1(\"%s\") = ",test->src);
		for (j = 0;j < 5; j++)
		      printf("%02x%02x%02x%02x",computed.digest[j] & 0xFF,
		      (computed.digest[j] >> 8) & 0xFF,
		      (computed.digest[j] >> 16) & 0xFF,
		      (computed.digest[j] >> 24) & 0xFF);
		printf("\r\n");
      /*
        printf("Testing %d/%zu...", i+1, LENGTH_OF_ARRAY(tests));
        printf("Message is: '%s'\r\n",test->src);
		for (i = 0;i<5;i++)
			printf("0x%x 0x%x\r\n",computed.digest[i],expected.digest[i]);
        if (memcmp(computed.digest, expected.digest, sizeof(Sha1Digest)))
        {
